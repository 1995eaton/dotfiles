...
var fl = require('fs').readFileSync(process.argv[2], 'utf8');
var lines = fl.split('\n');
lines = lines.map(function(e) {
  return e.replace(/cterm\S*/g, '')
    .replace(/gui/g, 'cterm')
    .replace(/#[0-9A-Fa-f]*/g, function(e) {
      return HEX_TERM(e.slice(1));
    })
    .replace(/\S*=([bfBF][gG]|undercurl)/g, '')
    .replace(/([=,])italic/g, '$1bold')
    .replace(/ctermsp=\S*/g, '');
});
lines = lines.filter(function(e) {
  return !/hi\s/.test(e.replace(/^\s*/, '')) ||
    e.indexOf('=') !== -1;
});
